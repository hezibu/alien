...
sink("bayesian-output-sink-file-delete")
mod_output <- suppressMessages(native_discovery(data = medfish,
                                                time_col = "time",
                                                alien_col = "aliens",
                                                native_col = "natives",
...
closeAllConnections()
file.remove("bayesian-output-sink-file-delete")

test_that("bayesian_example returns appropriate class object", {
  expect_true(inherits(mod_output, c("CmdStanMCMC", "CmdStanFit", "R6")))
})

test_that("native_discovery nee", {
